...
marker_length = 2 #float(input("マーカーの1辺の長さ（センチメートル）を入力してください："))
# 固定マーカーと動くマーカーのIDを定義
...
cap1.set(cv2.CAP_PROP_TILT, 0)  # 傾きを設定
...
def get_camera_capture():
    global frame, fixed_marker_center, fixed_marker_length, movable_marker_distances, movable_marker_centers
    while True:
        # カメラ画像の取得
        _, frame = cap.read()
        marker_centers, marker_lengths,_ = detect_aruco_markers(frame.copy())
        # 固定マーカーの座標と長さを取得
        ...
        for i, marker_id in enumerate(movable_marker_ids):
            ...
            if fixed_marker_center and movable_marker_center and fixed_marker_length and fixed_marker_length:
                distance_cm = calculate_distance(fixed_marker_center, movable_marker_center, fixed_marker_length)
...
dxl.PosCnt_Vbase(Motor_ID,p1 + 8500,experiment_motor_speed)
#outer_finger_dis
...
dxl.PosCnt_Vbase(Motor_ID,p2 - inner_finger_dis,experiment_motor_speed)
...
p3 = dxl.read(Motor_ID, dxl.Address.PresentPosition)
dxl.PosCnt_Vbase(Motor_ID,p3 + 1500,experiment_motor_speed)
#inner_finger_dis
t1 = current_time
